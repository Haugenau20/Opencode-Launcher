...
# endpoint, Bitbucket, Jira, GitLab, JFrog, Confluence, M-Files) is baked into
# the squid image, not this repo, so this can only show the bits start.sh itself
# knows: the configured LLM/Bitbucket/Jira/GitLab/JFrog/Confluence/M-Files hosts from .env, and
...
cmd_show_allowlist() {
  ...
  info "Confluence, M-Files) is enforced inside the squid image, not in this repo — this"
  info "report shows only what start.sh itself knows about: configured hosts +"
  info "local extensions."
  echo

  if [ -f "$ENV_FILE" ]; then
    local llm_base llm_host bb_user jira_base gl_user jfrog_base conf_base mfiles_base
    ...
    mfiles_base="$(get_env MFILES_BASE_URL)"
    if [ -n "$mfiles_base" ]; then
      info "M-Files: credentials configured (host is baked into the squid image, not visible here)"
    else
      info "M-Files: not configured (no MFILES_BASE_URL in $ENV_FILE)"
    fi
  else
...
allowlist_summary_line() {
  ...
  if [ "${extra_count:-0}" -gt 0 ]; then
    printf 'egress allowlist: LLM(%s) + Bitbucket/Jira/GitLab/JFrog/Confluence/M-Files (baked into image) + %s local extension file(s) — see ./start.sh --show-allowlist' \
      "$llm_host" "$extra_count"
  else
    printf 'egress allowlist: LLM(%s) + Bitbucket/Jira/GitLab/JFrog/Confluence/M-Files (baked into image) — see ./start.sh --show-allowlist' \
      "$llm_host"
  fi
}
