...
| File | Style | Covers |
| --- | --- | --- |
| `unit_helpers.bats` | sources `start.sh`, calls helpers directly | `derive_slug`, `sed_escape`, `set_env`/`get_env` round-trips, `find_free_port` |
| `cli.bats` | runs a sandboxed copy of `start.sh` as a subprocess | arg parsing & errors, Docker preflight messages, Artifactory auth handling, slug → per-project env, `--prod` and user-layer overlay wiring, the first-run secrets flow |
